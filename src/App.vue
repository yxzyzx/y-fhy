...
const shortCustomerName = name => (name || '未填名').trim().slice(0, 3)
const shortServiceName = name => {
  const text = (name || '').trim()
  if (text.includes('手')) return '手'
  if (text.includes('足') || text.includes('腳') || text.includes('脚')) return '足'
  if (text.includes('卸')) return '卸'
  return text.slice(0, 1) || '項'
}
const adminBookingServicesText = booking => (booking.services || []).map(shortServiceName).join('+')

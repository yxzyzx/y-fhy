...
const importYear=ref(new Date().getFullYear()),importing=ref(false),confirmingImport=ref(false),importMessage=ref(''),importPreview=ref([]),importOpen=ref(false)
...
function openEditMine(booking){Object.assign(editMy,{open:true,id:booking.id,date:booking.date,startTime:booking.startTime,selectedServices:[...(booking.serviceIds||[])],name:booking.name||'',phone:booking.phone||'',note:booking.note||'',message:''})}
...
async function saveMine(){editMy.message='';const res=await fetch(`/api/my-bookings/${editMy.id}`,{method:'PUT',headers:{'Content-Type':'application/json','x-owner-token':ownerToken},body:JSON.stringify(editMy)});const body=await res.json();if(!res.ok){editMy.message=body.message;return}editMy.open=false;await Promise.all([loadMine(),loadBookings()])}
...
function adminCellClick(day,hour){const existing=adminBookingFor(day,hour);if(existing){adminDetail.value=existing;return}Object.assign(adminCreate,{open:true,mode:'booking',date:toKey(day),startTime:`${String(hour).padStart(2,'0')}:00`,endTime:'22:00',wholeDay:true,serviceId:adminServices.value[0]?.id||'',name:'',phone:'',note:''})}
...
<template>
    ...
    <div class="content-stack">
      ...
      <aside id="booking-panel" class="booking-card">
        ...
        <form @submit.prevent="submit">
          ...
          <div class="booking-fields"><label>02 預約日期<input v-model="form.date" type="date" :min="toKey(today)" required></label><label>03 開始時間<select v-model="form.startTime" required><option value="" disabled>請選擇</option><option v-for="time in availableTimes" :key="time">{{ time }}</option></select></label><label>姓名<input v-model="form.name" maxlength="30" required></label><label>聯絡電話<input v-model="form.phone" type="tel" maxlength="20" required></label></div>
        ...
        <div class="table-wrap" :class="{loading}">
          <div class="schedule-table transposed">
            ...
            <template v-for="day in days" :key="toKey(day)">
              ...
              <button v-for="hour in hours" :key="toKey(day)+hour" class="slot" :class="bookingFor(day,hour)?'booked':'available'" :disabled="!!bookingFor(day,hour)" @click="selectSlot(day,hour)">
                <template v-if="bookingFor(day,hour)"><span v-if="isStart(bookingFor(day,hour),hour)" class="booking-text"><b>{{ bookingFor(day,hour).type==='leave'?'休假':'已被預約' }}</b></span><span v-else class="continuing">•••</span></template>
                <span v-else>可預約</span>
              </button>
            </template>
          </div>
        </div>
  ...
  <div v-else class="admin-page">
    <header class="admin-topbar"><div><span class="brand-mark">指尖日常</span><span class="admin-label">後台管理</span></div><div class="admin-nav-actions"><template v-if="adminLoggedIn"><button class="nav-icon-button" title="從 Google 預約表中匯入" aria-label="從 Google 預約表中匯入" @click="importMessage='';importSetupOpen=true"><span aria-hidden="true">⇩</span></button><button class="nav-icon-button" title="匯出為 Excel" aria-label="匯出為 Excel" @click="exportConfirmOpen=true"><span aria-hidden="true">⇧</span></button></template><button class="back-home-button" @click="closeAdmin">← 返回預約首頁</button></div></header>
    <main class="admin-main">
      ...
      <template v-else>
        ...
        <section class="admin-panel standalone">
          ...
          <div class="admin-section admin-schedule-section"><div class="section-head admin-schedule-head"><h2>預約表</h2><div class="week-nav"><button @click="moveMonth(-1)" aria-label="上個月">←</button><span>{{ monthLabel }}</span><button @click="moveMonth(1)" aria-label="下個月">→</button></div></div><div class="table-wrap"><div class="schedule-table transposed admin-schedule"><div class="corner">日期</div><div v-for="hour in hours" :key="hour" class="time-head">{{ String(hour).padStart(2,'0') }}:00</div><template v-for="day in days" :key="toKey(day)"><div class="date-row-head"><b>週{{ weekdays[day.getDay()] }}</b><span>{{ day.getMonth()+1 }}/{{ day.getDate() }}</span></div><button v-for="hour in hours" :key="toKey(day)+hour" class="slot" :class="adminBookingFor(day,hour)?'booked':'available'" @click="adminCellClick(day,hour)"><template v-if="adminBookingFor(day,hour)"><span v-if="isStart(adminBookingFor(day,hour),hour)" class="booking-text"><b>{{ adminBookingFor(day,hour).type==='leave'?'休假':adminBookingFor(day,hour).name }}</b><small>{{ adminBookingFor(day,hour).type==='leave'?(adminBookingFor(day,hour).note||'不開放預約'):adminBookingFor(day,hour).services.join('・') }}</small></span><span v-else class="continuing">•••</span></template><span v-else>＋</span></button></template></div></div></div>
        </section>
        ...
        <div v-if="importSetupOpen" class="admin-overlay"><section class="tool-dialog"><button class="admin-close" @click="importSetupOpen=false">×</button><div class="tool-dialog-icon" aria-hidden="true">⇩</div><h2>從 Google 預約表中匯入</h2><p class="muted">資料辨識完成後，會先讓妳確認可匯入與衝突的內容。</p><label>資料年份<input v-model.number="importYear" type="number" min="2020" max="2100"></label><p v-if="importMessage" class="message">{{ importMessage }}</p><div class="dialog-actions"><button @click="importSetupOpen=false">取消</button><button class="primary" :disabled="importing" @click="importGoogle">{{ importing?'辨識中…':'開始匯入' }}</button></div></section></div>
